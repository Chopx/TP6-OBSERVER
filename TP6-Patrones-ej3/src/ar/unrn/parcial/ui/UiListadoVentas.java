...
public class UiListadoVentas extends JFrame {
	 ...
	 * Create the frame.
	 */
	public UiListadoVentas(EstacionDeServicio estacion, LocalDateTime inicio, LocalDateTime fin) {

		String[] titulos = { "Fecha y hora", "Tipo Nafta", "Litros", "Monto", "Email Cliente" };
		...
		try {
			facturas = estacion.obtenerFacturasEntreFechas(inicio, fin);
		} catch (Exception ex) {
		...
		for (Factura f : facturas) {
			...
			modelo.addRow(new Object[] { fecha, f.verTipoDeCombustible(), f.cantidadDeLitros(), f.obtenerMonto(),
					f.verEmail() });
		}
